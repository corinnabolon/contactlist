 ...
 * the contacts array to the retrieved array--doesn't draw yet
 */
...
function drawContacts() {
  ...
  contacts.forEach(contact => {
    contactsTemplate += `
    <div class="contacts card ${contact.emergencyContact ? 'emergency-contact' : ''}">
    <h3>${contact.name}</h3>
...
function toggleAddContactForm() {
  /* Easy way: document.getElementById("new-contact-form").classList.toggle("hidden") */
  ...
  if (newContactFormElem.classList.contains("hidden")) {
    ...
    document.getElementById("add-contact-button").classList.add("hidden")
  } else {
    document.getElementById("new-contact-form").classList.add("hidden")
    document.getElementById("add-contact-button").classList.remove("hidden")
  }
}
